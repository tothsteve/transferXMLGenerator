...
const TemplateView: React.FC<TemplateViewProps> = ({ isOpen, onClose, template }) => {
  if (!template) return null;

  // Use real template data from the API
  const templateBeneficiaries: TemplateBeneficiaryDetail[] =
    template.template_beneficiaries?.map((tb) => ({
      id: tb.id,
  ...
  const totalAmount = templateBeneficiaries.reduce(
    (sum, b) => sum + (parseFloat(b.default_amount) || 0),
    0
  );
    ...
    <Dialog open={isOpen} onClose={onClose} maxWidth="lg" fullWidth>
      <DialogTitle>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography variant="h6">Sablon részletei</Typography>
